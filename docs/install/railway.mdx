...
- `https://<your-railway-domain>/openclaw` — Control UI (or `/` if no `gateway.controlUi.basePath` is set)

This matches the outcome of [Getting Started](/start/getting-started): a running gateway, auth configured, and Control UI access—via the web instead of the CLI.
...
- `RAILWAY_RUN_UID=0` (required for persistent storage — Railway volumes are root-owned; this runs the container as root so it can create `/data/.openclaw` and `/data/workspace`. Without it you get "Permission denied" and state falls back to `/tmp`, which does not persist.)
...
- `OPENCLAW_GATEWAY_TRUSTED_PROXIES=100.64.0.0/24` (recommended for Railway — fixes "pairing required" and "Health Offline" in the dashboard)
